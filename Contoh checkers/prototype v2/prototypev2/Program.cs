...
namespace ConsoleCheckers
{
    ...
    public class PieceManager
    {
        ...
        public void RemovePieceFromPlayerList(Player player1, Player player2, Position pos)
        {
            ...
            if (p != null) player1.DaftarBidak.Remove(p);
            else
            {
                p = player2.DaftarBidak.FirstOrDefault(x => x.CurrentPosition.Equals(pos));
                if (p != null) player2.DaftarBidak.Remove(p);
            }
        }
    }
            ...
            for (int r = 0; r < 8; r++)
            {
                ...
                for (int c = 0; c < 8; c++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            char ch;
                            if (p.Warna == "White") ch = p.IsRaja ? 'W' : 'w';
                            else ch = p.IsRaja ? 'B' : 'b';
                            Console.Write(ch + " ");
                        }
                    }
                }
    ...
    public class MoveExecutor
    {
        ...
        public bool MakeMove(Position from, Position to, ref Player currentPlayer)
        {
            ...
            // switch turn
            SwitchTurn(ref currentPlayer);
            return true;
        }

        private void SwitchTurn(ref Player currentPlayer)
        {
    ...
    public class GameState
    {
        ...
        {
            bool result = moveExecutor.MakeMove(from, to, ref CurrentPlayer);
            if (result)
            {
                CheckGameOver();
            }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (gameState.Status == "Playing")
            {
                ...
                // if last move was a capture and another capture is possible with same piece, keep same player
